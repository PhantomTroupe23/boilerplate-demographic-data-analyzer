...
def calculate_demographic_data(print_data=True):
    ...
    # Higher and lower education masks
    higher_ed = df['education'].isin(['Bachelors', 'Masters', 'Doctorate'])

    # Percentage with salary >50K for each group
    higher_education_rich = round((df[higher_ed]['salary'] == '>50K').mean() * 100, 1)
    lower_education_rich = round((df[~higher_ed]['salary'] == '>50K').mean() * 100, 1)
    ...
    rich_percentages = (rich_counts / country_counts * 100).round(1)
    highest_earning_country = rich_percentages.idxmax()
    highest_earning_country_percentage = rich_percentages.max()

    # Top occupation in India among rich
